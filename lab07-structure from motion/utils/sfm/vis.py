...
def PlotImagePairMatches(im1, im2, matches):
    ...
    pair_image[0:im2.image.shape[0],
               im2_offset: im2.image.shape[1] + im2_offset] = im2.image
    ...
    for i in range(matches.shape[0]):
        ...
        ax.plot([kp1[0], kp2[0] + im2_offset],
                [kp1[1], kp2[1]], 'g-', linewidth=1)
    ax.axis('off')
...
def PlotCameras(images, registered_images, ax=None):
	...
	for image_name in registered_images:
		...
		if(cnt > 6):
			color = 'g'
		cnt += 1
		ax = PlotCamera(R, t, ax, 0.5, color)
