...
def train(writer, logger):
    ...
    train_dataloader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=2)

    val_dataset = DataloaderCifar10(img_size=32, is_transform=False, split='val')
    val_dataset.load_data(args.root)
    val_dataloader = torch.utils.data.DataLoader(dataset=val_dataset, batch_size=args.batch_size, shuffle=False, num_workers=2)
        ...
        for step, data in tqdm(enumerate(train_dataloader)):
            ...
            if total_steps % args.val_step == 0:
                ...
                with torch.no_grad():
                    ...
                    # When there has better result, store model parameters as best_model 
                    if acc > best_acc:
                        best_acc = acc
                        state = {
                                "epoch": epoch,
